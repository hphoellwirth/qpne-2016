 ...
 *      -q      compute quasi-perfect equilibrium
 *      -P  #   denominator of perturbed game parameter epsilon (default 100)
 *              for quasi-perfect equilibrium computation
 */
...
#define WHICHFORMS 4      /* currently only SF (0), NF (1), added QF (3)  */
...
static  Bool bqpe = 0;          /* compute quasi-perfect NE     */
...
void infosf()
{
    ...
    if (bqpe)
    {
        lcpsize [QPFORM] = nseqs[1] + 2*(nisets[2]+1) + nseqs[2] + 2*(nisets[1]+1) ;
...
void inforesult (Bool bsf, Bool bnf, int priorseed, int seed)
{
    ...
    if (bqpe)
	    printf(formatstring, pivots [QPFORM], 
            (double) pivots [QPFORM]*100.0 / (double) lcpsize [QPFORM],
	        (double) timeused [QPFORM] / CLOCKUNITSPERSECOND,
	        mpdigits [QPFORM], eqsize [1] [QPFORM], eqsize [2] [QPFORM]);    
    else
    {
        if (bnf)
	    {
...
void infosumresult (Bool bsf, Bool bnf, int m)
{
    ...
    printf("         ");
    if (bqpe)
	        printf(formatstring, (double) sumpivots [QPFORM]/ mm, 
                (double) sumpivots [QPFORM]*100.0 /
                (double) (lcpsize [QPFORM] * mm),
	            (double) sumtimeused [QPFORM] / (CLOCKUNITSPERSECOND * mm),
	            (double) summpdigits [QPFORM] / mm, 
                (double) sumeqsize [1] [QPFORM] / mm, 
                (double) sumeqsize [2] [QPFORM] / mm);    
    else
    {    
        if (bnf)
	    {
...
void processgame (int whichform, Bool bnf, int docuseed)
{
    ...
    else if (whichform == QPFORM)
        {
        if (bcomment)
            printf("Generating and solving QPE form.\n");
        sfqpelcp(eps);
        }        
    else
	abort();        /* no RNF yet   */

    ...
    for (pl = 1; pl < PLAYERS; pl++)
	{
	    ...
	    {
	        transperturbsol(pl, solz + offset);
	        equilsize = properqpmixisets(pl, solz + offset);
	        /* the next is  offset  for player 2 */
	        offset = nseqs[1];
	    }	    
	    else
...
int main(int argc, char *argv[])
{
    ...
    int whichform = 0;  /* 0: SF, 1: NF, 2: RSF, 3:QPF          */
    ...
    /* parse options    */
    while ( (c = getopt (argc, argv, "A:bcdeEgG:il:m:M:noOprs:S:tqP:")) != -1)
	switch (c)
	    {
	    int x; 
	    case 'A':
...

    ...
    printf("    tableaus               %s [N],  option -t\n",
	    flemke.bouttabl ? "Y" : "N");
    printf("    Compute quasi-PE       %s [N],  option -q\n",
	    bqpe ? "Y" : "N");
	printf("    QPE game parameter %5d [0],  option -P # \n", eps);    	    
                    
    if (levels == -1)
	{
    ...
    if (levels == -3)
	{
	printf("Solving Mertens' simple example\n");
	mertensexample();
	}
	else
    if (levels == -4)
	{
	printf("Solving Mertens' voting example\n");			
	mertensvotingexample();
	}		
	else
    if (levels == -5)
	{
	printf("Solving example from van Damme\n");			
	vandammeexample();
	}
	else
    if (levels == -6)
	{
	printf("Solving variant of matching pennies\n");			
	matchingpennies();
	}					
    else
		...
		if (bgambit)
	        ...
	        for (priorcount = 0; priorcount < multipriors; priorcount++)
	        {
                ...
                if (boutprior)
        	        outprior();
        	    if (bqpe)
        	        processgame(QPFORM, bnf, seed + gamecount); 
        	    else
        	    {
	                if (bnf)
