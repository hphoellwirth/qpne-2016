...
void mptorat(mp num, mp den, int *result_num, int *result_den) 
{
    char nummp [MAX_DIGITS], trnumsmp [MAX_DIGITS]; 
    char denmp [MAX_DIGITS], trdensmp [MAX_DIGITS];
    int trsize = 8;     
...
void trunc(mp Na,mp Da)    /* truncate Na Da to 9 digits */
{
    char nummp [MAXINPUT], trnummp [MAXINPUT]; 
    char denmp [MAXINPUT], trdenmp [MAXINPUT];
    int trsize = 9;     
    
    memset(nummp, '\0', sizeof(nummp));
    memset(denmp, '\0', sizeof(denmp));
    memset(trnummp, '\0', sizeof(trnummp));
    memset(trdenmp, '\0', sizeof(trdenmp));
    ...
    if (strlen(denmp) >= strlen(nummp))
    {
        if (strlen(denmp) <= trsize)
        {
            strcpy(trdenmp, denmp);
            strcpy(trnummp, nummp);
        }
        else
        {
            strncpy(trdenmp, denmp, trsize);
            if (strlen(denmp) - strlen(nummp) > trsize) 
                strcpy(trnummp, '0');
            else
                strncpy(trnummp, nummp, trsize+strlen(nummp)-strlen(denmp));                 
        }        
    }
    else
    {
        ...
        {
            strncpy(trnummp, nummp, trsize);
            if (strlen(nummp) - strlen(denmp) > trsize) 
                strcpy(trdenmp, '0');
            else
                strncpy(trdenmp, denmp, trsize+strlen(denmp)-strlen(nummp));                  
        }        
    } 
